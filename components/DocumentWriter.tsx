      ...
      {isTrashOpen ? (
        ...
        <div className="flex flex-col md:flex-row gap-6 flex-1 min-h-0 pb-20 md:pb-0 overflow-y-auto md:overflow-visible overflow-x-hidden">
          ...
          <div ref={editorRef} className="w-full md:w-2/3 paper-panel rounded-lg bg-white border border-[var(--border-color)] shadow-inner relative flex flex-col min-h-[500px] mb-8">
             {/* Editor Toolbar */}
             <div className="h-auto md:h-12 py-2 md:py-0 bg-[var(--surface-color)] border-b border-[var(--border-color)] flex items-center justify-between px-4 flex-shrink-0 flex-wrap gap-2">
                <div className="flex items-center gap-2 overflow-hidden w-full md:w-auto mb-2 md:mb-0">
                  <span className="material-icons text-[var(--accent)]">edit</span>
                  <span className="font-serif font-bold text-[var(--text-primary)] truncate max-w-[200px] md:max-w-none">{activeSection?.title || 'No Section Selected'}</span>
                </div>

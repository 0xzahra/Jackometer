...
export const analyzeData = async (dataInput: string, tableData: string): Promise<AnalysisResult> => {
  ...
  const response = await ai.models.generateContent({
    model: 'gemini-2.0-flash', // Switched for speed/consistency
    contents: prompt,
    config: {
      responseMimeType: 'application/json',
      responseSchema: {
...
export const gradeEssay = async (essay: string, instruction: string): Promise<string> => {
    ...
    \n${ANTI_SLOP_PROMPT}`;
    const response = await ai.models.generateContent({ 
        model: 'gemini-2.0-flash', // Switched for speed
        contents: prompt,
        config: { tools: [{ googleSearch: {} }] }
    });
    return response.text || "";
...
export const reviewCareerDocument = async (docData: { text?: string, fileData?: string, mimeType?: string }): Promise<string> => {
  ...
  if (docData.fileData && docData.mimeType) {
    const response = await ai.models.generateContent({
      model: 'gemini-2.0-flash', // A model that properly accepts both PDF and Images
      contents: {
        parts: [
